...
del(['*.zip']).then(paths => {
  ...
  var output = fs.createWriteStream(path.join(__dirname, "/../", `${package.displayName}-v` + package.version + '.zip'));
  ...
  archive.directory(path.join(__dirname + '/../interface/'), "interface");
